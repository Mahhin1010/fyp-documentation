...
Traceability connects the requirements in this chapter with the implemented application features described later in the construction chapter. This is important because the project is not only a design proposal; it is a working Django-based system whose implementation can be mapped back to the stated functional and non-functional requirements.

\renewcommand{\arraystretch}{1.35}
\begin{longtable}{|p{2.6cm}|p{4.2cm}|p{6.9cm}|}
	\caption{Requirement-to-feature traceability matrix.}
	\label{tab:req_traceability}\\
	\hline
	\textbf{Requirement IDs} & \textbf{Feature Area} & \textbf{Implemented or Documented Realization} \\
	\hline
	\endfirsthead
	\hline
	\textbf{Requirement IDs} & \textbf{Feature Area} & \textbf{Implemented or Documented Realization} \\
	\hline
	\endhead
	SRS-1--SRS-7 & Authentication and session management & Django's built-in authentication views, the custom signup view, session middleware, CSRF protection, and password validation support account creation, login, logout, and protected access to application pages. \\ \hline
	SRS-8--SRS-13 & Category management & The Category model links every category to a user and enforces uniqueness through a user-name constraint. The category screen allows users to add and delete categories and displays admin-managed default suggestions. \\ \hline
	SRS-14--SRS-19 & CSV ingestion and validation & The transaction upload form accepts CSV files only, validates required columns, maps optional fields, parses dates and amounts, skips invalid rows, and stores valid transactions in bulk. \\ \hline
	SRS-20--SRS-24 & Intelligent sorting and persistence & Gemini API is used through a batching and validation service. The system sends transaction context and user categories, validates the JSON response, applies the confidence threshold, and persists category assignments through bulk updates. \\ \hline
	SRS-25--SRS-28 & Analytics dashboard & Metabase provides the BI layer. Django generates a signed dashboard embedding URL so that authenticated users can view visual analytics inside the application interface. \\ \hline
	SRS-29--SRS-33 & Administration & The Django admin interface, styled through Jazzmin, exposes users, categories, default categories, and transactions to authorized administrators for management and review. \\ \hline
	NFR Security & Data protection and isolation & User-owned Category and Transaction records include foreign keys to the authenticated user. Application views filter by request.user so one user's uploaded transactions and categories are not returned to another user. \\ \hline
	NFR Reliability & Error handling and recoverability & CSV errors are collected per row instead of stopping the entire upload. AI errors are reported through user-facing messages, and raw uploaded transactions remain available for retry if classification fails. \\ \hline
	NFR Performance & Efficient processing & Bulk insertion is used for uploaded transactions, and bulk update is used after classification. This avoids row-by-row database saves during high-volume CSV processing. \\ \hline
\end{longtable}
